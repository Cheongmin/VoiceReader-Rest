...
def test_photo_get_success(monkeypatch, flask_client):
...
def test_get_user(monkeypatch):
    ...
    user = controller.get_user(expected_user_id)

    assert expected_user_id == user['_id']
    ...
    class MockDb:
        class MockUsers:
            def find_one(self, query):
                return {
                    '_id': expected_user_id,
                    'fcm_uid': expected_fcm_uid
                }

        users = MockUsers()

    expected_user_id = 'VALID_USER_ID'
    expected_fcm_uid = 'VALID_FIREBASE_UID'

    monkeypatch.setattr(controller.mongo, 'db', MockDb())

    user_id = controller.get_user_id(expected_fcm_uid)
