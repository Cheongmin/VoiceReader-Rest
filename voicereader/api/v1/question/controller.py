...
class QuestionList(Resource):
    @jwt_required
    @api.doc(description='Request fetch questions')
    ...
    @jwt_required
    @api.doc(description='Request add new question')
    ...
    def post(self):
        ...
        if not allowed_file(sound_file.filename, SOUND_ALLOWED_EXTENSIONS):
            raise UnsupportedMediaType('Unsupport media type only mp3, m4a')
...
class Question(Resource):
    @jwt_required
    @api.doc(description='Request fetch question by question id')
    ...
    @jwt_required
    @api.doc(description='Request remove question by question id')
    @api.response(204)
    @api.response(400, 'Invalid question id')
    @api.response(401, 'Unauthorized access_token')
    @api.response(404, 'Not exists question')
